...
import { WhatsAppSessionSchema } from '../db/mongodb';
import { mongooseToJoi, validateSchema } from '../helpers';

export default class WhatsAppSessionValidator {
  private filterSchema = mongooseToJoi<IWhatsAppSessionAttributes>({
    schema: WhatsAppSessionSchema,
    isFilterSchema: true,
  });

  private createSchema = mongooseToJoi<IWhatsAppSessionAttributes>({
    schema: WhatsAppSessionSchema,
    requiredFields: ['userId', 'phone'],
  });

  private updateSchema = mongooseToJoi<IWhatsAppSessionAttributes>({
    schema: WhatsAppSessionSchema,
    // excludeFields: ['title'],
  });
